...
def _compare_generated_text_to_answers(pred_txt, reference_answers):
    ...
    for answer in unique_reference_answers:
        ...
        references = [answer]
        # print('answer', answer, 'prediction', predictions)
...
def _compare_generated_texts_to_answers(preds, reference_answers):
    ...
    pred_map = {preds: preds.lstrip().lower()}
    ...
    return {preds: results[pred_map[preds]]}
...
def _clean_sample(sample, tokenizer):
    ...
    ret['generations'] = [None] #* len(sample['most_likely_generation'])
    # print('tokenizer', tokenizer)
    if tokenizer is None:
        #for i, generation in enumerate(sample['generations']):
        for i, generation in enumerate(sample['most_likely_generation']):
            ret['generations'][i] = _clean_answer(generation, None, tokenizer)
    else:
        #for i, generation in enumerate(sample['generations_ids']):
        #for i, generation in enumerate(sample['most_likely_generation_ids']):
        i = 0
        generation = sample['most_likely_generation_ids']
        # print('i', i, generation)
        # print('ret', ret['generations'])
        ...
        ret['generations'][i] = _clean_answer(sample['most_likely_generation'], generation, tokenizer)
    #ret['generations'] = {k: [v[k] for v in ret['generations']] for k in ret['generations'][0].keys()}
    ret['generations'] = ret['generations'][0]
    return ret
...
def _get_rouge_sample(row, text_key=None):
    ...
    # print( _get_text(row['generations']), row['generations'])
    all_results = _compare_generated_texts_to_answers(all_preds, all_ans)
    curr = {'id': row['id']}
    # print('gt', _get_text(row['generations']))
    # print('all_results', all_results)
    #curr['generations'] = [all_results[_] for _ in _get_text(row['generations'])]
    curr['generations'] = [all_results[_get_text(row['generations'])]]
    return curr

def _get_rouge_sample_fake_label(row, text_key=None):
    _get_text = lambda x: x if text_key is None else x[text_key]
    # all_ans = [row['answer']]
    # if 'additional_answers' in row and row['additional_answers'] is not None:
    #     all_ans += row['additional_answers']
    all_ans = [row['fake_answer']]
    all_preds = _get_text(row['generations'])
    # print( _get_text(row['generations']), row['generations'])
    all_results = _compare_generated_texts_to_answers(all_preds, all_ans)
    curr = {'id': row['id']}
...
def _get_rouge(samples, clean:bool, logger=None, if_fake_label=False):
    text_key = 'text_cleaned' if clean else 'text'
    ret = []
    for sample in tqdm.tqdm(samples):
        #print('sample', sample)
        if if_fake_label:
            ret.append(_get_rouge_sample_fake_label(sample, text_key))
        else:

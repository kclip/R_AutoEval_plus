...
def find_prompts_cross_val(cand_prompts, loss_table, eval_template,
                 demos_template,
                 prompt_gen_data,
                 eval_data,
                 conf,
    ...
    prompts_cv, data_cv = generate.generate_prompts_for_cross_val(
    ...
    print('prompts cv', prompts_cv, 'data cv', data_cv)

    for ind_cv in range(len(prompts_cv)):
        prompt = prompts_cv[ind_cv]
        eval_data = data_cv[ind_cv]
        print('current prompt:', prompt)
        print('current eval data: ', eval_data)
        res = evaluate.evalute_prompts(prompt, eval_template, eval_data, demos_template, few_shot_data,
                                        conf['evaluation']['method'], conf['evaluation'])
        prompts, scores = res.sorted()
        print('prompt scores sorted', prompts, scores)
        if ind_cv == 0:
            ...
            if scores[0] > best_score:
                best_prompt = prompts[0]
                best_score = scores[0]
        print('ind cv: ', ind_cv, 'best score', best_score, 'best prompt:', best_prompt)
    return best_prompt


def find_prompts(cand_prompts, eval_template,
                 demos_template,
                 prompt_gen_data,
                 eval_data,
                 conf,
                 base_conf='configs/default.yaml',
                 few_shot_data=None,
                 prompt_gen_template=None, loaded_model=None, loaded_tokenizer=None):
    """
    Function to generate prompts using APE.
    Parameters:
        eval_template: The template for the evaluation queries.
    ...
    if cand_prompts is None:
        print('Generating prompts...')
        print('num exemplers: ', conf['generation']['num_subsamples'])
        prompts = generate.generate_prompts(
            prompt_gen_template, demos_template, prompt_gen_data, conf['generation'], loaded_model, loaded_tokenizer)
        print('Model returned {} prompts. Deduplicating...'.format(len(prompts)))
...
def get_hard_decision_from_prompts(prompts, eval_template, eval_data, demos_template, few_shot_data, conf,
                     base_conf='configs/default.yaml'):
    """
    ...
    #print('Evaluating prompts...')
    hard_prediction = evaluate.get_hard_decision_from_prompts(

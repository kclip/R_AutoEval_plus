...
fig, axs = plt.subplots(nrows=2, ncols=1, figsize=(4 / 1.1, 5 / 1.1))
...
for m in [3,4,5,6,7,8,9,10]:
    ...
    for ratio in [2,4,8]:
        for k1 in [16,64]:
            ...
            axs[0].scatter(avg_bitwidth, np.average(rel_loss)*100, marker=marker, c=color, label=config_str,alpha=0.6)
            axs[1].scatter(avg_bitwidth, qsnr, marker=marker, c=color, label=config_str, alpha=0.6)
        ind_ratio += 1
    ind_m += 1        

# unquan = np.load('./full_precision/score.npy')
axs[0].axhline(10, c='orange', linestyle='--')
axs[0].set_xlabel(r'average bitwidth')
axs[0].set_ylabel(r'performance drop [\%]')
# axs[1].legend()
axs[1].set_xlabel(r'avg. bitwidth')
axs[1].set_ylabel(r'QSNR [dB]')
...
plt.tight_layout()
path = Path('./vis_rel_completee.png')

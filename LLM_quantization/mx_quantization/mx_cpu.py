...
def mx_quant_cpu(model, mx_dict):
    m = mx_dict['m']
    k_1 = mx_dict['k1']
    k_2 = mx_dict['k2']
    d_1 = mx_dict['d1']
    ...
    for f in model.parameters():
        ...
        tmp, f.err, f.signal, _, _, per_layer_elements, per_layer_bits = MX_cpu(f.data.cpu(), m, k_1, k_2, d_1, d_2, None, None)
        f.data.copy_(tmp)
        del tmp
...
def MX_cpu(vec, m, k_1, k_2, d_1, d_2, s, s_g):
    total_bitwidth = 0
    ...
    def _reshape_with_padding(x, k_1):
        ...
        padded_x = torch.cat([x.flatten(), torch.zeros(pad_size, dtype=x.dtype)])

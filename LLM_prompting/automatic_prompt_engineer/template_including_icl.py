...
class EvalTemplate:
    ...
    def fill(self, prompt='',  input='', output=''):
        """
        Fills in the template with the given values.
        """
        return self.template.replace('[PROMPT]', prompt).replace('[INPUT]', input).replace('[OUTPUT]', output)
...
class DemosTemplate:
    ...
    def __init__(self, template, template_for_actual, delimiter='\n\n'):
        self.template = template
        self.delimiter = delimiter
        self.template_for_actual = template_for_actual

    def fill(self, data, input='', output=''):
        """
        ...
        #demos += "\n\nInput: [INPUT]\nOutput: [OUTPUT]"
        demos += self.template_for_actual.replace('[INPUT]', input).replace('[OUTPUT]', output)
        #demos.replace('[INPUT]', input).replace('[OUTPUT]', output)
        return demos
